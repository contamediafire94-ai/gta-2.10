...
bool ImGuiWrapper::initialize()
{
	 ...
	 * AddFontDefault() elimina essa dependência durante a inicialização
	 * e permite testar o restante do cliente.
	 */
	Log::addParameter("Font mode: ImGui default");

	ImFontConfig fontCfg;
	fontCfg.GlyphExtraSpacing.x = UISettings::outlineSize();

	ImFont* font = io.Fonts->AddFontDefault(&fontCfg);

	if (font == nullptr)
	{
		Log::addParameter("Default font error", font);
		return false;
	}

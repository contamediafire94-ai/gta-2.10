...
stFile* NvFOpen(const char* r0, const char* r1, int r2, int r3)
{
    ...
    if(!strncmp(r1+12, "mainV1.scm", 10))
    {
        ...
        sprintf(path, "%sSAMP/mainV1.scm", g_pszStorage);
    ...
    // Diagnostico: testa se um arquivo copiado manualmente para a raiz
    // de Android/data/com.samp.mobile/files pode ser lido pelo proprio app.
    if (!strncmp(r1, "texdb/samp/samp.txt", 20))
    {
        char testPath[255]{};
        sprintf(testPath, "%smenu_test.txt", g_pszStorage);
        ...
        if (testFile)
        {
            FLog("MANUAL ROOT TEST OK | %s", testPath);
            fclose(testFile);
        }
        else
        {
            int testErr = errno;
            FLog("MANUAL ROOT TEST FAIL | path=%s | errno=%d | %s",
                 testPath, testErr, strerror(testErr));
        }
    }

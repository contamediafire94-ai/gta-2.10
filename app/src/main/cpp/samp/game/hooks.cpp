...
void CEntity_Render_hook(CEntityGTA* pEntity)
{
    ...
    // V10 DIAGNOSTICO:
    // O V9 mostrou que CCamera::Fade() retorna normalmente e o SIGBUS acontece
    // somente depois, no primeiro render, sempre com o mesmo ultimo modelo: 1268.
    // Pulamos APENAS esse LOD para confirmar se o crash pertence a esse modelo
    // especifico ou se o problema e mais amplo no pipeline 3D.
    if (pEntity)
    {
        ...
        if (modelId == 1268)
        {
            static bool loggedModel1268Skip = false;
            if (!loggedModel1268Skip)
            {
                FLog("V10: skipping GTA model 1268 (LODlBd1) for SIGBUS isolation");
                loggedModel1268Skip = true;
            }
            return;
        }

...
stFile* NvFOpen(const char* r0, const char* r1, int r2, int r3)
{
    ...
    // Redireciona o arquivo de idioma para a pasta SAMP.
    // O Android estava falhando ao abrir TEXT/AMERICAN.GXT diretamente.
    if (!strncmp(r1, "TEXT/AMERICAN.GXT", 17))
    {
        sprintf(path, "%sSAMP/AMERICAN.GXT", g_pszStorage);

...
stFile* NvFOpen(const char* r0, const char* r1, int r2, int r3)
{
    ...
    // Redireciona o arquivo de idioma para o caminho real existente no Android.
    // O jogo pede TEXT/AMERICAN.GXT, mas os arquivos originais estão em Text/american.gxt.
    if (!strncmp(r1, "TEXT/AMERICAN.GXT", 17))
    {
        sprintf(path, "%sText/american.gxt", g_pszStorage);

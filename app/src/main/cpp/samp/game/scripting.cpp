...
void InitScripting()
{
    FLog("InitScripting: BEGIN");
    FLog("InitScripting: before new GAME_SCRIPT_THREAD");

    gst = new GAME_SCRIPT_THREAD;

    FLog("InitScripting: after new | gst=%p", (void*)gst);

    if (!gst)
    {
        FLog("InitScripting: ERROR - gst is NULL");
        return;
    }

    FLog("InitScripting: before memset | size=%u",
         (unsigned int)sizeof(GAME_SCRIPT_THREAD));

    memset(gst, 0, sizeof(GAME_SCRIPT_THREAD));

    FLog("InitScripting: after memset");
    FLog("InitScripting: DONE");
}

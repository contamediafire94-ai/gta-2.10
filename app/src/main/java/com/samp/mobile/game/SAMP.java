...
public class SAMP extends GTASA implements
    ...
    private void prepareAmericanGxt(Context context) {
        ...
        File sampDir = new File(root, "SAMP");

        if (!sampDir.exists() && !sampDir.mkdirs()) {
            Log.e(TAG, "Nao foi possivel criar a pasta SAMP: " + sampDir.getAbsolutePath());
            return;
        }

        // Nome novo para não depender do AMERICAN.GXT antigo,
        // que estava retornando EACCES / Permission denied.
        File target = new File(sampDir, "AMERICAN_APP.GXT");
